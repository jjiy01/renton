...
create table tb_domain (
	domain_id bigint generated by default as identity(start with 1) primary key,
	domain_name varchar(256) not null unique,
	description varchar(4000)
);

create table tb_acl_entry (
	entry_id bigint generated by default as identity(start with 1) primary key,
	group_id bigint not null,
	domain_id bigint not null,
	permission integer not null,
	constraint fk_acl_entry_group
	foreign key(group_id) references tb_groups(group_id),
	constraint fk_acl_entry_domain
	foreign key(domain_id) references tb_domain(domain_id)
);
